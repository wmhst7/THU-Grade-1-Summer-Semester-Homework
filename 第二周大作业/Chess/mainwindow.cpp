...
void MainWindow::on_actionServer_triggered()
{
    ...
    if( dialog->exec() ==  QDialog::Accepted) {
        ...
        if(!server -> listen(QHostAddress::Any, port)){
            QMessageBox::information(this, "QTcpServer", "服务器端监听失败！");
            return;
        }
        socket = new QTcpSocket();
        connect(server, SIGNAL(newConnection()), this, SLOT(NewConnection()));
        QMessageBox::information(this, "QTcpServer","Success!");
    }
}
...
void MainWindow::ObserveTime(){
    ...
    if(Time == 0){
        ...
        kernel->status = 2;//Server is black, black lost
        timer->stop();
        SendLost();
    }
}
...
void MainWindow::on_actionCloseServer_triggered()
{
    ...
    server = nullptr;
    socket = nullptr;
    QMessageBox::information(this, "QTcpServer","Close Server!");
